...
private constructor(context: Context, private val mParentView: ViewGroup?, private val mCallback: Callback?) {
     ...
     * callback's [Callback.onEdgeTouched] and
     ...
     * [Callback.tryCaptureView] will not be asked
    ...
    fun captureChildView(childView: View, activePointerId: Int) {
        if (childView.parent !== mParentView) {
            throw IllegalArgumentException("captureChildView: parameter must be a descendant " + "of the ViewDragHelper's tracked parent view ("
                    + mParentView + ")")
        }
    ...
    fun settleCapturedViewAt(finalLeft: Int, finalTop: Int): Boolean {
        ...
        return forceSettleCapturedViewAt(finalLeft, finalTop, VelocityTrackerCompat.getXVelocity(mVelocityTracker!!, activePointerId).toInt(),
    ...
    fun flingCapturedView(minLeft: Int, minTop: Int, maxLeft: Int, maxTop: Int) {
        ...
        mScroller.fling(capturedView!!.left, capturedView!!.top,
                ...
                VelocityTrackerCompat.getYVelocity(mVelocityTracker!!, activePointerId).toInt(), minLeft, maxLeft, minTop, maxTop)
    ...
    private fun checkNewEdgeDrag(delta: Float, odelta: Float, pointerId: Int, edge: Int): Boolean {
        ...
        if (mInitialEdgesTouched!![pointerId] and edge != edge || mTrackingEdges and edge == 0 || mEdgeDragsLocked!![pointerId] and edge == edge
                || mEdgeDragsInProgress!![pointerId] and edge == edge || absDelta <= touchSlop && absODelta <= touchSlop) {
            return false
        }
